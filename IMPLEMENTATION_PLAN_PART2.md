# IMPLEMENTATION PLAN ΓÇö FlashBalanceAI (Part 2 of 2)
## Phases 5ΓÇô9: AWS Integration ΓåÆ Experiments ΓåÆ Stats ΓåÆ Paper ΓåÆ Demo + Full Timeline
...
> **Part 2 covers:** Phase 5 (AWS Integration), Phase 6 (Experiments E1ΓÇôE10), Phase 7 (Statistical Validation), Phase 8 (Paper Preparation), Phase 9 (Demo/Teardown), Full Timeline Table, Research Integrity Checklist, Contradictions Between PDFs.

---

## PHASE 5 ΓÇö AWS INTEGRATION

**Duration:** 5ΓÇô7 days | **[COSTS MONEY during testing]**  
**Prerequisites:** Phase 4 complete (all AWS resources running and verified).

---

### T5.1 ΓÇö CloudWatch State Collector Lambda [CRITICAL PATH][COSTS MONEY ΓÇö minimal]
...
```
4. **ΓÜá∩╕Å NOTE on 10s polling:** AWS EventBridge minimum rate is 1 minute. For 10s polling, use:
...
**Acceptance Criteria:** After deployment, `aws s3 cp s3://flashbalanceai-{ACCOUNT_ID}/state/current.json -` shows a valid 23-element state vector with plausible CPU values (0ΓÇô1).  
**Costs Money?** Lambda free tier covers this easily. EventBridge: first 1M events free ΓåÆ **effectively free**.

---

### T5.2 ΓÇö PPO Inference Server + Inference Coordinator Lambda [CRITICAL PATH]
...
> **Architecture decision (from cost review 2026-08-20):** Pure Lambda inference is NOT used. SB3 + PyTorch unzipped is ~300 MB, exceeding Lambda's 250 MB unzipped package limit. The EC2 inference server approach is the confirmed primary path. See PRD ┬º12.2 and ADR-001 D14.

**Component A ΓÇö EC2 Inference Server (`src/aws/inference_server.py`):**
```python
...
# Load model at startup ΓÇö not per-request
s3 = boto3.client('s3')
...
**Component B ΓÇö Inference Coordinator Lambda (`src/aws/inference_coordinator.py`):**
```python
# Lightweight Lambda ΓÇö does NOT contain SB3. < 5 MB package.
...
```
Trigger: EventBridge rule `rate(1 minute)` ΓÇö every 30s not achievable without Step Functions; 1-minute polling is sufficient for the 30s state-collection cycle.
...
### T5.3 ΓÇö Scaling Trigger Lambda [PARALLEL][FREE TIER]
...
def lambda_handler(event, context):
    ...
    if source == 'burst_preemptive':
        # Proactive: burst_indicator triggered ΓÇö add 2 instances immediately
...
4. Add scale-in alarm (CPU < 30% for 5 periods ΓåÆ scale in by 1).
...
**Costs Money?** Lambda free tier ΓåÆ **Free**. CloudWatch alarm: 5 free alarms ΓåÆ **Free**.

---

### T5.4 ΓÇö CloudWatch Custom Metrics from Backend [PARALLEL]

**Objective:** Flask backend publishes **exactly 2 custom metrics per instance** (QueueDepth, ResponseTimeEMA) to CloudWatch every 30s. Total: 8 custom metrics across 4 instances ΓÇö within the 10-metric free tier.
...
**Costs Money?** First 10 custom metrics free; we need 8 (2 per instance ├ù 4) ΓåÆ **Free**.

---

### T5.5 ΓÇö End-to-End Integration Test [CRITICAL PATH]

**Objective:** Verify the complete revised data flow: JMeter ΓåÆ ALB (direct, no API Gateway) ΓåÆ EC2 ΓåÆ CloudWatch ΓåÆ Lambda State Collector ΓåÆ S3 ΓåÆ Lambda Inference Coordinator ΓåÆ EC2 Inference Server ΓåÆ ALB update ΓåÆ DynamoDB.
...
3. Run a 5-minute JMeter test at baseline load (100 req/s) **targeting ALB DNS directly** ΓÇö no API Gateway.
...
6. Check DynamoDB: `routing_decisions` table accumulating rows with valid `action` (0ΓÇô3) values.
...
**Estimated Time:** 3 hours
**Acceptance Criteria:** All 6 checklist steps confirmed. Cost during 1-hour integration test: ~$0.04ΓÇô$0.06 (ALB + EC2). Results documented in `decisions/integration_test_report.md`.
**Costs Money?** **YES** ΓÇö ALB + EC2 active. Estimate: ~$0.05 for a 1-hour test.

---

## PHASE 6 ΓÇö EXPERIMENTAL SETUP

**Duration:** 7ΓÇô10 days | **[COSTS MONEY]** | **Prerequisites:** T5.5 (integration test) passed.

**ΓÜá∩╕Å RESEARCH INTEGRITY:** No results from these experiments may be reported in the paper until  
...
### E1 ΓÇö Baseline Traffic (1├ù Load) [CRITICAL PATH]
...
| **Algorithms** | PPO, DQN, RR, WRR, LC, Threshold ΓÇö all 6 |
...
**Costs Money?** **YES** ΓÇö 6 algorithms ├ù 5 runs ├ù 13 min = ~6.5 hours of ALB + EC2 time ΓåÆ ~$0.50.

---

### E2 ΓÇö 10├ù Traffic Spike [CRITICAL PATH]

| Parameter | Value |
|-----------|-------|
| **Purpose** | Primary experiment ΓÇö main paper result |
| **Traffic profile** | Warm-up (2min, 100rps) ΓåÆ Pre-burst (1min, 300rps) ΓåÆ Spike onset (10s, 0ΓåÆ1000rps) ΓåÆ Peak (5min, 1000rps) ΓåÆ Cooldown (5min) |
| **Duration** | 13 minutes per run |
| **Repetitions** | 5 (seeds 132ΓÇô136) |
| **Algorithms** | PPO, DQN, RR, WRR, LC, Threshold ΓÇö all 6 |
...
**Expected output:**
- PPO: P95 < 500ms during peak (EXPECTED ΓÇö unvalidated until experiment runs)
- Threshold: P95 likely 800msΓÇô2000ms during spike onset (EXPECTED)
- RR/LC: P95 likely 500msΓÇô1500ms depending on load distribution (EXPECTED)
...
| **Duration** | 13 minutes per run |
| **Repetitions** | 5 (seeds 137ΓÇô141) |
| **ΓÜá∩╕Å PREREQUISITE** | E2 must complete successfully first. If E2 shows infrastructure ceiling issues, redesign before running E3. |

**Steps:** Same as E2, `peak_users=5000`.

**Expected output:** RR/Threshold likely exceed SLA at 50├ù due to queue overflow. PPO may also degrade but maintain lower P95 than baselines (EXPECTED). Infrastructure ceiling (max 8 t2.micro) likely reached.
...
### E4 ΓÇö 100├ù Traffic Spike (Conditional)

| Parameter | Value |
|-----------|-------|
| **Purpose** | Upper bound; expected all methods exceed SLA |
| **Traffic profile** | peak_users=10000 |
| **Duration** | 8 minutes (shortened ΓÇö infrastructure stress) |
| **Repetitions** | 3 (seeds 127ΓÇô129) ΓÇö reduced due to expected infrastructure ceiling |
| **Condition** | Run ONLY if E3 showed PPO maintaining < 2000ms P95 at 50├ù. Otherwise document as out-of-scope. |
...
### E5 ΓÇö Repeated Flash-Sale Patterns

| Parameter | Value |
|-----------|-------|
| **Purpose** | Test recovery between bursts; validate that PPO adapts across multiple events |
| **Traffic profile** | 3 consecutive 10├ù bursts, 2-minute gap between each |
| **Duration** | ~45 minutes per run |
| **Repetitions** | 5 |
| **Algorithms** | PPO, DQN, Threshold (primary comparison ΓÇö most relevant for repeated events) |
...
**Expected output:** PPO shows improved handling of 2nd/3rd burst compared to 1st (EXPECTED ΓÇö PPO adapts online). Threshold scales out/in each time with cold-start delay.

**Costs Money?** ~$1.50 (longer duration).

---

### E6 ΓÇö Noisy/Transient Spikes

| Parameter | Value |
|-----------|-------|
| **Purpose** | Test whether PPO over-reacts to short transient noise within baseline |
| **Traffic profile** | Baseline 100rps with random 2├ù spikes lasting 10ΓÇô30 seconds, noise_std=0.40 |
| **Duration** | 13 minutes per run |
| **Repetitions** | 5 |
| **Algorithms** | PPO, DQN, Threshold (Threshold over-reaction expected) |

**Expected output:** PPO should NOT trigger scale-out for transient 2├ù spikes (burst_indicator dampens noise). Threshold may oscillate. This tests the H-MAS noise-filtering insight from Mohar's document (simplified via burst_indicator).

**Costs Money?** ~$0.50.

---

### E7 ΓÇö Cold-Start / Scaling Response Time

| Parameter | Value |
|-----------|-------|
| **Purpose** | Measure time from burst onset to N+1 instances being healthy in target group |
| **Setup** | Start with desired-capacity=2 (below normal). Trigger 10├ù burst. Measure time until 3rd instance is healthy. |
| **Duration** | 10 minutes per run |
| **Repetitions** | 5 (PPO proactive) + 5 (Threshold reactive) |

**Measurement protocol:**
1. Record tΓéÇ = timestamp of burst onset (JMeter ramp-up start).
2. Record tΓéü = timestamp when ASG desired-capacity changes (CloudWatch event).
3. Record tΓéé = timestamp when new instance shows `healthy` in ALB target health.
4. Scaling reaction time = tΓéé ΓêÆ tΓéÇ (user-visible impact window).
5. For PPO proactive: burst_indicator fires at tΓéÇ, immediate scale call ΓåÆ expect tΓéü Γëê tΓéÇ + 5s.
6. For Threshold: CPU must breach 70% for 2├ù30s = 60s minimum ΓåÆ tΓéü Γëê tΓéÇ + 90s.
...
### E8 ΓÇö Cost / Performance Trade-off

| Parameter | Value |
|-----------|-------|
| **Purpose** | Understand how changing wΓéé (utilisation weight) affects AWS cost vs P95 latency |
| **Config** | 3 configurations: (a) wΓéé=0.10 (performance focus), (b) wΓéé=0.20 (default), (c) wΓéé=0.40 (cost focus) |
| **Traffic** | 10├ù spike (same as E2) |
| **Duration** | 13 minutes per run ├ù 3 configs |
| **Repetitions** | 3 per config = 9 total runs |

**For each config:**
1. Retrain PPO with modified reward weights (3 new training runs, ~4 hours each ΓÇö consider running on Colab overnight).
2. Run E2-equivalent experiment with the retrained model.
3. Record: P95 latency, average CPU utilisation, estimated AWS cost (instances ├ù hours).
...
**Costs Money?** ~$0.50 for experiments + potential SageMaker training ($0.46 ├ù 4hrs ├ù 3 configs = ~$0.55).

---

### E9 ΓÇö DRL Algorithm Comparison (PPO vs DQN)

| Parameter | Value |
|-----------|-------|
| **Purpose** | Core research question RQ2: does PPO outperform DQN for flash-sale routing? |
| **Traffic** | 10├ù and 50├ù bursts (E2 + E3 scenarios) |
| **Duration** | Same as E2/E3 ΓÇö use same runs (no additional AWS cost) |
...
3. Statistical comparison: paired t-test on P95 latency (PPO vs DQN), ╬▒ = 0.05.
4. Report: mean P95, 95% CI, Cohen's d effect size, p-value.

**ΓÜá∩╕Å INTEGRITY NOTE:** Do not adjust test seeds or filter runs after seeing results. Report all 5 repetitions regardless of outcome.

**Costs Money?** No additional cost (uses E2/E3 data).

---

### E10 ΓÇö Ablation Study of Reward Components

| Parameter | Value |
|-----------|-------|
| **Purpose** | Validate each reward component's contribution (RQ3) |
| **Variants** | (a) Full reward (default), (b) No R_lat (wΓéü=0), (c) No R_util (wΓéé=0), (d) No R_tput (wΓéâ=0), (e) No R_sla (wΓéä=0) |
| **Traffic** | 10├ù burst |
| **Duration** | 13 minutes per run |
| **Repetitions** | 3 per variant = 15 total runs |

**Steps:**
1. For each variant, modify `ppo_config.yaml` reward weights ΓåÆ retrain PPO (run on Colab to save SageMaker cost).
2. Evaluate on same 3 test seeds as E2.
3. Compare P95 latency, SLA violation rate, CPU utilisation across variants.
4. Expected: removing R_sla (wΓéä=0) should increase P99 latency most (EXPECTED ΓÇö unvalidated).
...
## PHASE 7 ΓÇö RESULTS AND STATISTICAL VALIDATION

**Duration:** 4ΓÇô5 days | **[LOCAL ONLY]**

---

### T7.1 ΓÇö Raw Data Collection and Verification [CRITICAL PATH]
...
2. Verify completeness: each `.jtl` should have approximately 7800 rows (13 min ├ù 10 req/s average).
...
**Acceptance Criteria:** Every experiment (E1ΓÇôE10) has raw data files for the specified number of repetitions. No file is empty or corrupted.

---

### T7.2 ΓÇö Statistical Analysis [CRITICAL PATH][LOCAL ONLY]

**Objective:** Compute all reported statistics with correct methodology.

**Steps ΓÇö execute in `notebooks/03_results_analysis.ipynb`:**
...
### T7.3 ΓÇö Figures and Plots [LOCAL ONLY][PARALLEL with T7.2]

**Objective:** Create all paper-quality figures.

**Required figures:**

1. **Figure 1 ΓÇö System Architecture Diagram** (draw.io or matplotlib, vector format)
2. **Figure 2 ΓÇö Traffic Profile** (matplotlib, shows warm-up/burst/peak/cooldown phases for 10├ù and 50├ù)
3. **Figure 3 ΓÇö Reward Convergence Curves** (PPO and DQN, episode reward vs training steps, with shaded 95% CI)
4. **Figure 4 ΓÇö Latency CDF** (P5ΓÇôP99 latency CDF for all 6 algorithms at 10├ù burst)
5. **Figure 5 ΓÇö P95 Latency Comparison Bar Chart** (E1, E2, E3 results, grouped by algorithm)
6. **Figure 6 ΓÇö CPU Utilisation Timeline** (time-series CPU during 10├ù burst, one line per instance)
7. **Figure 7 ΓÇö Scaling Reaction Time** (timeline bar: PPO proactive vs Threshold reactive, E7)
8. **Figure 8 ΓÇö Ablation Study** (E10 results, bar chart for each reward variant)

**Tools:** matplotlib 3.8 + seaborn 0.13 (LaTeX font rendering: `plt.rcParams.update({'text.usetex': True})`)

**Format requirements for paper:**
- All figures: PDF vector format (not PNG ΓÇö PDF preserves crispness in IEEE/ACM papers)
- Font size ΓëÑ 8pt in figures (IEEE minimum)
- Figure dimensions: single-column (3.5 in wide) or double-column (7.16 in wide)
- Save to `experiments/analysis/figures/`

---

### T7.4 ΓÇö Reproducibility Verification [CRITICAL PATH]
...
2. Run E2 with seeds 132ΓÇô133 (2 repetitions only for verification).
3. Compare P95 latency to recorded results ΓÇö must be within ┬▒5%.
...
## PHASE 8 ΓÇö CONFERENCE PAPER PREPARATION

**Duration:** 7ΓÇô10 days | **[LOCAL ONLY]**

---

### T8.1 ΓÇö Paper Outline and Section Assignment [CRITICAL PATH]
...
| References | Agrima | 15ΓÇô20 refs | Ongoing |

**Target conference format:** IEEE 2-column format (IEEEtran). Download template from ieeetran.com.  
**Target length:** 6ΓÇô8 pages (typical for IEEE Cloud/ICDCS workshops).

---

### T8.2 ΓÇö Algorithm Pseudocode [PARALLEL]
...
Initialize: PPO actor ╧Ç_╬╕, critic V_╧å, rollout buffer B
for episode = 1 to N_episodes do
  obs ΓåÉ E.reset(seed)
  for step = 0 to K do
    a_t Γê╝ ╧Ç_╬╕(┬╖|s_t)          // Sample action from policy
    s_{t+1}, r_t ΓåÉ E.step(a_t)  // Execute action
    B.store(s_t, a_t, r_t, s_{t+1})
  end for
  Compute GAE advantages: ├é_t from B
  for epoch = 0 to n_epochs do
    for mini-batch m in B do
      r_t(╬╕) ΓåÉ ╧Ç_╬╕(a_t|s_t)/╧Ç_╬╕_old(a_t|s_t)
      L^CLIP ΓåÉ E[min(r_t(╬╕)├é_t, clip(r_t(╬╕),1-╬╡,1+╬╡)├é_t)]
      Update ╬╕, ╧å via Adam on ΓêÆL^CLIP + cΓéüL^VF ΓêÆ cΓééS[╧Ç_╬╕]
    end for
  end for
...
### T8.3 ΓÇö Related Work Section [PARALLEL]

**Objective:** Write the Related Work section citing the 10 surveyed papers plus 5ΓÇô8 additional papers.

**Mandatory citations (from PDFs):**

From Devkanti's survey:
1. Zhou et al. (2024) ΓÇö DRL survey, Springer AI Review
2. Hu et al. (2025) ΓÇö D4PG network routing, IEEE Trans. Ind. Informatics
3. Chen et al. (2026) ΓÇö MDP DRL for e-commerce, Elsevier ESWA
4. Femminella & Reali (2024) ΓÇö PPO for serverless HPA, MDPI Computers
5. Funika et al. (2023) ΓÇö PPO-LSTM heterogeneous cloud, Springer J. Supercomputing

From Agrima's survey:
6. Jian et al. (2024) ΓÇö DRS K8s scheduler, Wiley SPE
7. Yamsani & Chenna Reddy (2026) ΓÇö SLA-DRL, Nature Scientific Reports
8. Zhou et al. (2024) ΓÇö TS-SDTRA PPO+DQN, Elsevier Computer Communications
9. Li et al. (2024) ΓÇö Distributional RL batch scheduling, IEEE TPDS

From Mohar's survey:
10. MAS-H2 paper ΓÇö Hierarchical Multi-Agent autoscaling, Kubernetes cluster metrics

**Additional papers recommended for strong related work:**

| Paper | Why Needed |
|-------|-----------|
| Mnih et al. (2015) ΓÇö DQN (Nature) | Foundational citation for DQN baseline |
| Schulman et al. (2017) ΓÇö PPO (arXiv) | Foundational citation for PPO |
| Sutton & Barto (2018) ΓÇö RL textbook | General RL background |
| Busta et al. (2023/2024) ΓÇö AWS Auto Scaling production analysis | Grounds AWS-specific claims |
| A flash-sale/thundering-herd paper from e-commerce literature | Establishes flash-sale problem severity |

**ΓÜá∩╕Å INTEGRITY NOTE:** Only cite papers you have actually read. Do not fabricate DOIs, volume numbers, or page ranges. Verify all citations against Google Scholar before submission.

---

### T8.4 ΓÇö Results Section and Tables [CRITICAL PATH after T7.2]

**Main results table (Table 1):**

```
Table 1: Performance comparison under 10├ù flash-sale burst traffic.
Results are mean ┬▒ std over 5 independent runs (test seeds 132-136).
ΓÇá denotes statistically significant improvement over PPO at ╬▒=0.05 (paired t-test).

Algorithm   | Mean Lat (ms) | P95 (ms)    | P99 (ms)    | Fail%  | CPU%   | Cost/1M
------------|---------------|-------------|-------------|--------|--------|--------
PPO (ours)  | [fill]┬▒[fill] | [fill]┬▒[fill] | [fill]┬▒[fill] | [fill] | [fill] | [fill]
DQN         | [fill]┬▒[fill] | [fill]┬▒[fill]ΓÇá| ...         | ...    | ...    | ...
RoundRobin  | [fill]┬▒[fill] | [fill]┬▒[fill]ΓÇá| ...         | ...    | ...    | ...
WRR         | ...           | ...          | ...         | ...    | ...    | ...
LeastConn   | ...           | ...          | ...         | ...    | ...    | ...
Threshold   | [fill]┬▒[fill] | [fill]┬▒[fill]ΓÇá| ...         | ...    | ...    | ...
```
...
### T8.5 ΓÇö Limitations, Threats to Validity, Future Work [PARALLEL]
...
## PHASE 9 ΓÇö FINAL DEPLOYMENT AND DEMO

**Duration:** 2ΓÇô3 days | **[COSTS MONEY ΓÇö minimal]**

---

### T9.1 ΓÇö Read-Only Public Demo [OPTIONAL][COSTS MONEY]
...
2. Run 1├ù baseline traffic only (no burst ΓÇö avoids cost spike).
...
**Cost:** 2 EC2 t2.micro + 1 ALB running for demo duration (e.g., 4 hours during demo session) ΓåÆ ~$0.15.

---

### T9.2 ΓÇö Monitoring Dashboard [PARALLEL]
...
app.layout = html.Div([
    html.H1("FlashBalanceAI ΓÇö Live Monitor"),
...
def update_graphs(n):
    ...
    cpu_vals = state[:4]
    resp_vals = [v * 500 for v in state[12:16]]  # resp_ema ΓåÆ ms
...
```
2. Run locally: `python src/metrics/visualiser.py` ΓåÆ open `http://localhost:8050`.

---

### T9.3 ΓÇö Teardown Procedure [CRITICAL PATH ΓÇö must execute after all experiments]
...
**ΓÜá∩╕Å DO NOT DELETE S3 until all experimental results are downloaded locally AND committed to git.**
...
| Expected improvements | 30% P95 reduction, 85% utilisation, 20ΓÇô25% cost saving | Not quantified | Zero-downtime scaling | **Mark all as EXPECTED until experimentally validated** |

### Unsupported Claims to Remove or Verify

1. Γ¥î "30% reduction in P95 response time" ΓÇö REMOVE from PRD/paper until measured.
2. Γ¥î "85%+ resource utilization" ΓÇö REMOVE until measured.
3. Γ¥î "20ΓÇô25% cost savings" ΓÇö REMOVE until measured.
4. Γ¥î "First DRL load balancer on AWS" ΓÇö WEAKEN to "within our surveyed literature..."
5. Γ¥î "Zero-downtime scaling" ΓÇö Unverifiable without production deployment.
6. ΓÜá∩╕Å Alibaba Cluster Trace cited as primary dataset for flash-sale validation (Mohar) ΓÇö **INCORRECT**. Alibaba trace is batch jobs. Correct to: supplementary only.
7. ΓÜá∩╕Å NASA/ClarkNet logs from 1995 cited by Devkanti (P5) as a limitation in the source paper ΓÇö do not use these datasets in our project.
...
| T1.2 Flask backend | Mohar | T1.1 | 2h | Day 2ΓÇô3 | None | src/backend/app.py |
| T1.3 Gymnasium env | Devkanti | T1.1, T2.1 | 6h | Day 3ΓÇô4 | None | flash_sale_env.py |
| T1.4 Baselines | Agrima | T1.1 | 3h | Day 2ΓÇô3 | None | 4 baseline classes |
| T1.5 Metrics | All | T1.1 | 2h | Day 2ΓÇô3 | None | collector.py |
| T2.1 Traffic generator | Devkanti | T1.1 | 4h | Day 3ΓÇô4 | None | traffic_generator.py |
| T2.2 JMeter plans | Mohar | T1.2 | 3h | Day 4ΓÇô5 | None | .jmx test plans |
| T2.3 Alibaba dataset | Agrima | T1.1 | 2h | Day 4ΓÇô5 | None | alibaba_processed.csv |
| T3.1 PPO training | Devkanti | T1.3, T2.1 | 9h | Day 5ΓÇô7 | $0ΓÇô0.50 (SageMaker) | ppo_flash_v1.zip |
| T3.2 DQN training | Agrima | T1.3, T2.1 | 9h | Day 5ΓÇô7 | $0ΓÇô0.50 | dqn_flash_v1.zip |
...
| T4.4 EC2 + ASG | Mohar | T4.2, T1.2 | 4h | Day 10ΓÇô11 | ΓÜá∩╕Å Medium | Running EC2 pool |
| T4.5 ALB | Mohar | T4.4 | 2h | Day 11 | ΓÜá∩╕Å High ($/hr) | ALB DNS |
| T4.6 DynamoDB | Agrima | T4.1 | 0.5h | Day 9 | Free | routing_decisions |
| T4.7 SageMaker (optional) | Any | T4.2 | 2h | Day 10 | ΓÜá∩╕Å Low ($0.23) | model in S3 |
| T5.1 State collector Lambda | Agrima | T4.3, T4.4, T4.5 | 5h | Day 12ΓÇô13 | ΓÜá∩╕Å Low | state/current.json |
| T5.2 PPO inference | Devkanti | T5.1, T4.3 | 6h | Day 13ΓÇô14 | ΓÜá∩╕Å Low | Routing decisions |
...
| T5.5 Integration test | All | T5.1ΓÇôT5.4 | 3h | Day 14ΓÇô15 | ΓÜá∩╕Å Low ($0.05) | E2E verified |
| E1 Baseline traffic | Agrima | T5.5 | 4h | Day 16 | ΓÜá∩╕Å Medium ($0.50) | E1 results CSVs |
| E2 10├ù spike | All | E1 | 6h | Day 16ΓÇô17 | ΓÜá∩╕Å Medium ($0.50) | E2 results CSVs |
| E3 50├ù spike | All | E2 | 5h | Day 17ΓÇô18 | ΓÜá∩╕Å Medium ($0.60) | E3 results CSVs |
| E4 100├ù (conditional) | Devkanti | E3 | 3h | Day 18 | ΓÜá∩╕Å Medium ($0.30) | E4 results |
| E5 Repeated bursts | Mohar | E2 | 5h | Day 18ΓÇô19 | ΓÜá∩╕Å Medium ($1.50) | E5 results |
| E6 Noisy spikes | Agrima | E2 | 4h | Day 18ΓÇô19 | ΓÜá∩╕Å Low ($0.50) | E6 results |
| E7 Scaling response | All | E2 | 4h | Day 19ΓÇô20 | ΓÜá∩╕Å Low ($0.50) | E7 results |
| E8 Cost/perf trade-off | Devkanti | E2 | 8h | Day 20ΓÇô22 | ΓÜá∩╕Å Medium ($1.00) | E8 results |
| E9 DRL comparison | All | E2, E3 | 2h | Day 17 (reuse data) | None | E9 analysis |
| E10 Ablation study | All | E2 + retraining | 10h | Day 21ΓÇô24 | ΓÜá∩╕Å Medium ($1.00) | E10 results |
| T7.1 Data collection | Agrima | All E-tasks | 3h | Day 25 | None | Raw CSVs in S3 |
| T7.2 Statistical analysis | Devkanti | T7.1 | 6h | Day 25ΓÇô26 | None | Stats table |
| T7.3 Figures | Mohar | T7.1 | 6h | Day 25ΓÇô27 | None | PDF figures |
...
| T8.2 Pseudocode | Devkanti | T3.1 | 2h | Day 27ΓÇô28 | None | Algorithm blocks |
| T8.3 Related work | Agrima | T8.1 | 5h | Day 28ΓÇô30 | None | Related work draft |
| T8.4 Results section | Devkanti | T7.3 | 4h | Day 28ΓÇô30 | None | Results draft |
| T8.5 Limitations + discussion | Mohar + all | T8.4 | 3h | Day 30ΓÇô31 | None | Discussion draft |
| Paper review + revisions | All | T8.3ΓÇôT8.5 | 8h | Day 31ΓÇô35 | None | Submission-ready draft |
| T9.1 Demo (optional) | Any | T5.5 | 3h | Day 35 | ΓÜá∩╕Å Low ($0.15) | Live demo URL |
| T9.2 Dashboard | Agrima | T5.4 | 4h | Day 34ΓÇô35 | None | Dashboard running |
| T9.3 Teardown | Devkanti | After experiments | 1h | Day 36 | None | All resources deleted |

**Total estimated hours:** ~170ΓÇô200 engineering hours  
**Total AWS cost:** $15ΓÇô35 (conservative to realistic)
...
```
Week 1 (Days 1ΓÇô7):   Phase 0 + Phase 1 + Phase 2 (all local, no AWS cost)
Week 2 (Days 8ΓÇô14):  Phase 3 (training) + Phase 4 (AWS setup)
Week 3 (Days 15ΓÇô22): Phase 5 (integration) + Phase 6 (experiments E1ΓÇôE7)
Week 4 (Days 23ΓÇô28): Phase 6 cont. (E8ΓÇôE10) + Phase 7 (stats + figures)
Week 5 (Days 29ΓÇô35): Phase 8 (paper writing) + Phase 9 (demo + teardown)
```
...
```
T0.1 ΓåÆ T0.2 ΓåÆ T1.1 ΓåÆ T1.3 ΓåÆ T2.1 [environment ready]
                              Γåô
                         T3.1 [PPO trained]
                              Γåô
                         T3.3 [local eval gate check]
                              Γåô
T4.1 ΓåÆ T4.2 ΓåÆ T4.4 ΓåÆ T4.5 [AWS infrastructure]
                         Γåô
                    T5.1 ΓåÆ T5.5 [integration verified]
                         Γåô
                    E1 ΓåÆ E2 ΓåÆ E3 [primary experiments]
                         Γåô
                    T7.2 ΓåÆ T7.3 [statistics + figures]
                         Γåô
                    T8.4 ΓåÆ paper submission
```
...
**Target: 10 days**
1. Days 1ΓÇô2: T0.1, T0.2, T1.1 (env setup)
2. Days 2ΓÇô4: T1.2, T1.3, T2.1 (backend + Gymnasium env + traffic generator)
3. Days 4ΓÇô7: T3.1 (PPO training ΓÇö run overnight)
4. Days 7ΓÇô8: T4.1, T4.2, T4.3, T4.4, T4.5 (AWS stack)
5. Days 8ΓÇô10: T5.1, T5.2, T5.5 (integration + basic inference running)
...
The bottleneck is PPO/DQN training time (~4ΓÇô5 hours each) and the minimum 5-repetition experimental runs with ALB active.
...
| Task | Gate Condition |
|------|---------------|
| T3.2 (DQN training) | Must use same FlashSaleEnv version as T3.1 ΓÇö freeze environment before starting |
| T4.4 (EC2 launch) | T4.1 billing alarms must be set first |
| T5.5 (integration test) | All of T5.1ΓÇôT5.4 must be individually verified first |
| E2 (10├ù experiments) | E1 must complete with all 6 algorithms; no NaN metrics |
| E3 (50├ù experiments) | E2 must show PPO outperforming RR by > 10% P95 |
| E4 (100├ù experiments) | E3 must show infrastructure can handle 50├ù without all instances crashing |
...
- EC2 t2.micro ├ù 4 (backend) + 1 (inference server)
- ALB ├ù 1
- Lambda ├ù 3 (state collector, scaling trigger, lightweight coordinator)
- CloudWatch (custom metrics, alarms)
- S3 ├ù 1 bucket
- DynamoDB ├ù 1 table
- API Gateway ├ù 1 REST API
- ASG ├ù 1
- IAM ├ù 2 roles
...
**Experiments:** E1ΓÇôE10 as defined, minimum 5 repetitions each, 95% CI, paired t-test for primary claims.

**Why this architecture wins:**
1. PPO's clipped objective makes it stable under the high reward variance of flash-sale burst traffic ΓÇö no equivalent guarantee from DQN or DDPG
2. AWS-native deployment fills the verified Gap G2 from all 10 surveyed papers
3. DQN comparison fills Gap G3 (only one prior paper does this, in SDN sim ΓÇö not AWS, not flash-sale)
4. Synthetic traffic generator is reproducible, calibrated, and avoids legal/privacy issues with real data
5. Total AWS budget is $15ΓÇô35 ΓÇö achievable with a student account within 5 weeks
