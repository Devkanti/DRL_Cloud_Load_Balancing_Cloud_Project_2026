...
Primary baselines are Round Robin, Least Connections, threshold-based autoscaling, and DQN. The system is designed for reproducible experimental validation on a student AWS account with minimum cost, targeting conference publication (IEEE/ACM Cloud/ICDCS/CCGRID 2026ΓÇô2027).
...
1. **Thundering herd onset:** Traffic escalates from baseline (~100 req/s) to peak (~10,000ΓÇô100,000 req/s) within 2ΓÇô10 seconds ΓÇö faster than any reactive autoscaler's cold-start latency (AWS ASG scale-up: 2ΓÇô5 minutes).
2. **Transient noise within the burst:** Millisecond-level traffic spikes within the overall burst cause standard DRL agents to oscillate ΓÇö overreacting to noise, thrashing auto-scaling, and degrading performance.

**Consequence:** Server CPU saturation ΓåÆ latency spikes ΓåÆ request queue overflow ΓåÆ HTTP 500/503 errors ΓåÆ revenue loss. Industry reports (cited in Devkanti's survey) estimate that every second of additional latency during a flash sale reduces conversions by 7%.

**Formal problem:** Given a stream of incoming HTTP requests at time *t* with arrival rate ╬╗(t) that follows a flash-sale burst profile, distribute requests across *N* heterogeneous backend EC2 instances such that P95 latency remains Γëñ 500 ms, throughput is maximised, SLO violations are minimised, and AWS operational cost is minimised ΓÇö all simultaneously, in real time, without manual reconfiguration.
...
- Threshold-based autoscaling (CPU > 70% ΓåÆ add instance) reacts after the spike, not before it.
...
- Tests DRL under flash-sale burst traffic (10├ùΓÇô100├ù spikes within seconds)
...
**Why PPO (and not DQN or DDPG)?** ΓåÆ See Section 10.
...
| O4 | Reduce P95 response time by ΓëÑ 30% vs threshold-based autoscaling during 10├ù spike | Measured across ΓëÑ 5 independent experimental runs |
| O5 | Maintain ΓëÑ 85% CPU utilisation during peak load (avoid over-provisioning) | Measured from CloudWatch during peak window |
| O6 | Reduce AWS operational cost by 20ΓÇô25% vs static over-provisioned baseline | Measured from AWS Cost Explorer during equivalent experiment runs |
...
### In Scope
- PPO agent for request-level routing across N EC2 backend instances (N configurable: 2ΓÇô8)
...
- Synthetic flash-sale traffic generator (normal ΓåÆ spike ΓåÆ peak ΓåÆ cooldown)
...
- Container orchestration (ECS, EKS) ΓÇö EC2-based only
...
> **Architecture revision (2026-08-20):** API Gateway removed from primary data path (JMeter targets ALB DNS directly). CloudFront removed. SageMaker demoted to optional fallback. See PRD ┬º12.0 for rationale.
...
```
                         ...
                         Γöé              AWS Cloud (us-east-1)               Γöé
                         Γöé                                                  Γöé
 JMeter (local)          Γöé  ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ   Γöé
 flash-sale traffic ΓöÇΓöÇΓöÇΓöÇΓû╢Γöé  Γöé      ALB (HTTP:80, weighted TG)          Γöé   Γöé
                         Γöé  ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö¼ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ   Γöé
                         Γöé                      Γöé routes to                Γöé
                         Γöé  ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓû╝ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ   Γöé
                         Γöé  Γöé        EC2 Backend Pool (ASG)            Γöé   Γöé
                         Γöé  Γöé   server-0  server-1  server-2  server-3 Γöé   Γöé
                         Γöé  Γöé   t2.micro  t2.micro  t2.micro  t2.micro Γöé   Γöé
                         Γöé  ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö¼ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ   Γöé
                         Γöé                      Γöé metrics (30s)            Γöé
                         Γöé  ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓû╝ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ   Γöé
                         Γöé  Γöé  CloudWatch                              Γöé   Γöé
                         Γöé  Γöé  CPUUtil (built-in) + 8 custom metrics   Γöé   Γöé
                         Γöé  Γöé  Alarms ΓåÆ Lambda ΓåÆ ASG scale-out/in      Γöé   Γöé
                         Γöé  ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö¼ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö¼ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ   Γöé
                         Γöé         Γöé (every 30s)             Γöé (alarm)     Γöé
                         Γöé  ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓû╝ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ   ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓû╝ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ  Γöé
                         Γöé  ΓöéLambda: State       Γöé   ΓöéLambda: Scaling   Γöé  Γöé
                         Γöé  ΓöéCollector           Γöé   ΓöéTrigger           Γöé  Γöé
                         Γöé  ΓöéΓåÆ S3 current.json   Γöé   ΓöéΓåÆ ASG desired cap Γöé  Γöé
                         Γöé  ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö¼ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ   ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ  Γöé
                         Γöé         Γöé (every 30s)                           Γöé
                         Γöé  ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓû╝ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ Γöé
                         Γöé  ΓöéLambda: Inference Coordinator               Γöé Γöé
                         Γöé  Γöé  reads S3 state ΓåÆ POST /infer ΓåÆ            Γöé Γöé
                         Γöé  Γöé  EC2 inference server (t2.micro)           Γöé Γöé
                         Γöé  Γöé  ΓåÆ elbv2 update ALB weights                Γöé Γöé
                         Γöé  Γöé  ΓåÆ DynamoDB log                            Γöé Γöé
                         Γöé  ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ Γöé
                         Γöé                                                  Γöé
                         Γöé  ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ   ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ  Γöé
                         Γöé  Γöé Amazon S3    Γöé   Γöé SageMaker (optional)   Γöé  Γöé
                         Γöé  Γöé models/      Γöé   Γöé ml.t2.medium           Γöé  Γöé
                         Γöé  Γöé state/       Γöé   Γöé fallback training only Γöé  Γöé
                         Γöé  Γöé results/     Γöé   ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ  Γöé
                         Γöé  ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ                               Γöé
                         ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ
```

**Revised Data Flow:**
1. JMeter (local) generates HTTP traffic following flash-sale burst profile ΓåÆ **directly to ALB DNS** (HTTP:80)
2. ALB forwards requests to backend EC2 instances based on current target group weights
3. Backend EC2 instances process requests ΓåÆ return response ΓåÆ ALB ΓåÆ JMeter records latency
4. Flask backend publishes 2 custom CloudWatch metrics (QueueDepth, ResponseTimeEMA) every 30s per instance
5. Lambda State Collector (every 30s): reads CloudWatch metrics ΓåÆ builds 23-dim state vector ΓåÆ writes `state/current.json` to S3
6. Lambda Inference Coordinator (every 30s): reads S3 state ΓåÆ `POST /infer` to EC2 inference server ΓåÆ inference server returns action ΓåÆ coordinator updates ALB target weights via `elbv2.modify_target_group_attributes` ΓåÆ logs to DynamoDB
7. CloudWatch alarm (CPU > 70% for 2├ù30s) ΓåÆ Lambda Scaling Trigger ΓåÆ ASG SetDesiredCapacity
...
The load balancing problem is formulated as a Markov Decision Process (S, A, P, R, ╬│):
...
- **Discount factor ╬│:** 0.99 (long-horizon optimisation)

**Decision epoch:** Every incoming request (event-driven) or every ╬öt = 100 ms (time-driven). For this project: **time-driven at 100 ms intervals** to avoid per-request Lambda cost explosion.

### 9.2 State Space

The state vector **s_t Γêê Γä¥^(5N+3)** where N = number of active backend instances:

```
s_t = [
  cpu_util[1..N],         # CPU utilisation for each instance (0ΓÇô1), shape: (N,)
  active_conn[1..N],      # Active HTTP connections per instance, normalised (0ΓÇô1), shape: (N,)
  queue_depth[1..N],      # Request queue depth per instance, normalised (0ΓÇô1), shape: (N,)
  resp_time_ema[1..N],    # Exponential moving avg response time per instance (0ΓÇô1), shape: (N,)
  health_status[1..N],    # Binary: 1=healthy, 0=unhealthy, shape: (N,)
  arrival_rate_norm,      # Current request arrival rate ╬╗(t) normalised against baseline (0ΓÇô10+)
  burst_indicator,        # Derived: ╬╗(t)/╬╗_baseline > 2 ΓåÆ 1, else 0
  time_since_last_spike   # Seconds since last burst onset, normalised (0ΓÇô1)
]
```

**State dimensions:** For N=4 backend instances ΓåÆ 4├ù5 + 3 = **23-dimensional state**

**Source of state observations:**
- `cpu_util`: CloudWatch `CPUUtilization` metric per EC2 instance (60s default ΓåÆ custom 10s metric)
...
- At inference: agent selects action *a*, ALB weights are adjusted to route 100% traffic to instance *a* for the next 100 ms window. In production, a weighted split would be more realistic ΓÇö this is noted as a limitation.
...
```
R(t) = wΓéü┬╖R_lat(t) + wΓéé┬╖R_util(t) + wΓéâ┬╖R_tput(t) + wΓéä┬╖R_sla(t)
```

**Components:**

**R_lat(t) ΓÇö Response time penalty:**
```
R_lat(t) = ΓêÆ(avg_response_time(t) / latency_target) + 1
         = ΓêÆ(L_t / 200ms) + 1
```
Range: large positive when L_t << 200ms, negative when L_t > 200ms.

**R_util(t) ΓÇö Resource utilisation reward:**
```
R_util(t) = avg_cpu_util(t) ΓêÆ |avg_cpu_util(t) ΓêÆ util_target|
...
|--------|-------|---------------|
| wΓéü (latency) | 0.40 | Primary user-facing metric during flash sales |
| wΓéé (utilisation) | 0.20 | Cost control ΓÇö avoid idle over-provisioning |
| wΓéâ (throughput) | 0.20 | Ensure requests are served, not just distributed |
| wΓéä (SLA) | 0.20 | Hard guardrail on tail latency SLA |
...
for each time step t = 0, 1, 2, ...:
    ...
    2. PPO policy ╧Ç_╬╕(a | s_t) ΓåÆ sample action a_t
...
- Actor network (policy): 2-layer MLP, [64, 64] hidden units, ReLU activations ΓåÆ softmax output over N actions
- Critic network (value function): 2-layer MLP, [64, 64] hidden units, ReLU ΓåÆ scalar value
- Shared trunk (optional): single 64-unit layer ΓåÆ separate actor/critic heads

**Training hyperparameters (initial values):**

| Parameter | Value | Source |
|-----------|-------|--------|
| Learning rate | 3├ù10Γü╗Γü┤ (Adam) | Schulman et al. standard |
| Clip ratio ╬╡ | 0.2 | PPO paper default |
| Value loss coefficient cΓéü | 0.5 | Standard |
| Entropy coefficient cΓéé | 0.01 | Encourages exploration |
| GAE ╬╗ | 0.95 | Standard |
| Discount ╬│ | 0.99 | Long horizon |
| Rollout steps K | 2048 | Stable-Baselines3 default |
| Mini-batch size | 64 | Standard |
| Update epochs | 10 | PPO standard |
| Max training steps | 2├ù10Γü╢ | Monitor convergence |
...
2. Generate synthetic flash-sale episode (30 min simulated ΓåÆ ~18,000 decision steps at 100ms)
...
2. Deploy as AWS Lambda function (128MB ΓåÆ 512MB if needed) or EC2 t2.micro
3. On each 100ms tick: call CloudWatch `get_metric_statistics` for all instances
4. Assemble state vector, run `policy.predict(state)` ΓåÆ action (< 2ms inference)
...
| Spike onset | 10 sec | 1,000ΓÇô10,000 req/s | Flash sale opens |
| Peak | 5 min | 2,000ΓÇô5,000 req/s | Sustained peak load |
| Cooldown | 5 min | 500 ΓåÆ 100 req/s | Gradual return to baseline |
| Total episode | ~13 min | ΓÇö | One complete flash-sale event |

**Burst multipliers tested:** 10├ù, 50├ù (primary), 100├ù (if infrastructure permits)
...
Arrival rate ╬╗(t) is modelled as a **superposition of baseline Poisson traffic and a log-normal burst**:

```
╬╗(t) = ╬╗_baseline + ╬╗_burst ┬╖ LogNormal(╬╝=log(peak_rate), ╧â=0.3) ┬╖ Gaussian_envelope(t; t_onset, ╧â_burst)
```

Where:
- `╬╗_baseline = 100 req/s`
- `╬╗_burst` = multiplier (10├ù, 50├ù, 100├ù ╬╗_baseline)
- `t_onset` = flash-sale start time (randomised ┬▒ 30s for test robustness)
- `╧â_burst = 60s` (flash sale lasts approximately 5 minutes at peak)
...
**Proactive pre-scaling trigger:** If `burst_indicator = 1` (arrival rate > 2├ù baseline) AND `time_since_last_spike < 600s` ΓåÆ Lambda pre-scales by +2 instances before CPU hits threshold.

**Reactive scaling:** Standard CloudWatch alarm: CPU > 70% for 2 consecutive 30s periods ΓåÆ Lambda calls `autoscaling:SetDesiredCapacity` (+1 instance). CPU < 30% for 5 consecutive periods ΓåÆ scale in (-1 instance).
...
4. ALB routes using current weight vector [wΓéü, wΓéé, ..., wN] where wß╡ó Γê¥ capacity of instance i
5. Sticky sessions DISABLED (stateless routing for clean measurement)

### 9.12 Monitoring/Feedback Loop

```
CloudWatch (10s metrics) ΓåÆ Lambda state collector ΓåÆ S3 state buffer
                                                        Γåô
...
|-----------|-----|-----|------|
| Action space | Discrete Γ£ô | Both Γ£ô | Continuous only |
...
PPO-Clip maximises:
```
L^CLIP(╬╕) = E_t[min(r_t(╬╕)┬╖├é_t, clip(r_t(╬╕), 1ΓêÆ╬╡, 1+╬╡)┬╖├é_t)]
```

Where:
- `r_t(╬╕) = ╧Ç_╬╕(a_t|s_t) / ╧Ç_╬╕_old(a_t|s_t)` ΓÇö probability ratio between new and old policy
- `├é_t` ΓÇö Generalised Advantage Estimate (GAE): `├é_t = ╬ú_{l=0}^Γê₧ (╬│╬╗)^l ┬╖ ╬┤_{t+l}`, `╬┤_t = r_t + ╬│V(s_{t+1}) ΓêÆ V(s_t)`
- `╬╡ = 0.2` ΓÇö clip ratio preventing large policy updates

The clip constraint solves the **policy collapse** problem that makes vanilla policy gradient methods unstable for bursty environments with high reward variance ΓÇö a critical property for flash-sale traffic where reward signals swing dramatically between phases.

**Full PPO objective:**
```
L(╬╕) = L^CLIP(╬╕) ΓêÆ cΓéü┬╖L^VF(╬╕) + cΓéé┬╖S[╧Ç_╬╕](s_t)
```
...
1. **Continuous action space mismatch:** Our ALB routing is inherently discrete (select one of N target groups). DDPG forces a continuous ΓåÆ discrete mapping that adds unnecessary complexity.
...
1. **Complexity vs benefit ratio:** H-MAS requires designing, training, and coordinating multiple agents with a hierarchy ΓÇö doubling implementation complexity for uncertain benefit.
...
|---------|--------|----------|-------------|-------------|
| Alibaba Cloud Cluster Trace | github.com/alibaba/clusterdata | Mohar | ~5 GB, 1M+ records, 12 features, CPU/memory/latency/request size, CSV/JSON | Useful for pre-training; NOT flash-sale traffic ΓÇö batch job cluster trace |
...
**ΓÜá∩╕Å CRITICAL FLAG:** NASA HTTP logs and ClarkNet logs are from 1995. Funika et al. (2023) use them as a limitation (cited in Devkanti's survey). These datasets are explicitly NOT suitable for flash-sale validation. Do not use them as primary datasets.

**ΓÜá∩╕Å FLAG:** Alibaba Cluster Trace contains batch job scheduling data, not real-time request routing data. It can be used to pre-train the agent on general load patterns but NOT as the primary evaluation dataset.
...
**Basis:** Calibrated against published e-commerce traffic reports (Amazon Prime Day 2023, Alibaba 11.11 2023 traffic pattern papers) ΓÇö specific citations to be added in paper.
...
| `arrival_rate_per_sec` | float | ╬╗(t) at this timestamp |
...
| `burst_multiplier` | float | Current ╬╗(t)/╬╗_baseline |
...
|-------|-----------|-------------|-------|
| Training | 70% | Agent training episodes | 42ΓÇô111 (70 episodes) |
| Validation | 15% | Hyperparameter tuning, early stopping | 112ΓÇô126 (15 episodes) |
| Test | 15% | Final evaluation, reported results | 127ΓÇô141 (15 episodes) |
...
> **Cost Optimisation Note (added 2026-08-20):** Section 12.0 documents a structured cost review. The core research methodology ΓÇö PPO-based adaptive load balancing, MDP formulation, reward function, state/action space, and experimental scope E1ΓÇôE10 ΓÇö is **unchanged**. Only infrastructure choices that carry unnecessary cost without research benefit have been revised. See ADR-001 D12/D13/D14 for the change log.
...
|---------|---------------|--------|
| EC2 t2.micro (backend ├ù N) | **KEEP ΓÇö required** | Backend servers are the experiment subject. Cannot be replaced. |
| ALB | **KEEP ΓÇö required** | PPO routing is implemented by adjusting ALB target weights via `elbv2` API. The ALB is the routing mechanism being studied. Replacing it with Nginx would make the PPOΓåÆALB integration ΓÇö a core novelty claim ΓÇö untestable on AWS. |
| Lambda (state collector) | **KEEP ΓÇö required** | Lightweight (128 MB, <1s), runs on free tier. Builds the 23-dim state vector from CloudWatch metrics. |
| Lambda (scaling trigger) | **KEEP ΓÇö required** | Lightweight, free tier. Implements the proactive pre-scaling mechanism (H8 hypothesis). |
| CloudWatch (EC2 built-in metrics) | **KEEP ΓÇö required** | CPUUtilization is a built-in free metric. Source of 4 of the 5 state vector components. |
| CloudWatch (custom metrics) | **KEEP ΓÇö reduced** | Originally 20+ metrics; reduced to 8 (2 per instance ├ù 4 instances). Stays within the 10 free custom metrics limit. See ┬º12.2. |
| S3 | **KEEP ΓÇö required** | Model storage, state buffer, experiment results. Total usage < 5 GB ΓåÆ free tier. |
| DynamoDB | **KEEP ΓÇö required** | Routing decision audit trail required for reward verification and reproducibility. Free tier is sufficient. |
| IAM | **KEEP ΓÇö required** | Always free. Required for least-privilege security. |
| SNS | **KEEP ΓÇö required** | 1 topic for billing alarm. Free tier is sufficient. |
| ASG | **KEEP ΓÇö required** | Proactive and reactive scaling are core to E7 experiment (cold-start time). No cost beyond EC2. |
| **SageMaker** | **OPTIONAL ΓÇö not primary** | Local training (laptop/Colab) is the primary path. SageMaker only if local training fails. See ┬º12.2. |
| **API Gateway** | **OPTIONAL ΓÇö not required** | JMeter can target the ALB DNS directly. API Gateway adds no research value and consumes 1M free-tier calls. Removed from the primary data flow; retained as an optional entry point. See ┬º12.2. |
...
| **EC2 inference server** | **KEEP ΓÇö revised** | Retained (one t2.micro), but instance is stopped between experiment sessions to minimise billing. |

**Cost reduction summary:**

| Change | Original estimated cost | Revised estimated cost |
|--------|------------------------|----------------------|
| Remove API Gateway from critical path | ~$0 (free tier) | $0 ΓÇö issue eliminated entirely |
| Remove CloudFront | ~$0 (free tier) | $0 ΓÇö issue eliminated entirely |
| Reduce custom CloudWatch metrics | ~$3/month (20 metrics) | ~$0 (Γëñ 10 metrics, free tier) |
| SageMaker: optional only | $0.25ΓÇô$0.50/training run | $0 primary path (local/Colab) |
| ALB idle time: strict stop/start | ~$5/month if left running | ~$1ΓÇô2 (active experiment hours only) |
| EC2 stop/start discipline | ~$5.22/month | ~$2ΓÇô3 (active hours only) |
| **Total revised estimate** | **$15ΓÇô35** | **$5ΓÇô15** |
...
|---|---------|--------------|------|----------------|
| 1 | EC2 | t2.micro ├ù 4 backend + 1 inference | Backend servers (Flask e-commerce simulator) + PPO inference server | Free tier 750 hrs/mo; beyond: ~$0.0116/hr/instance ΓÇö stop all instances between sessions |
| 2 | ALB | 1 ALB, 1 HTTP listener | Routes traffic to backend EC2 instances; PPO updates target weights via `elbv2` boto3 API | ~$0.0225/hr fixed + $0.008/LCU-hr ΓÇö **not free tier**; delete when not experimenting |
| 3 | Lambda | Python 3.11, 128 MB, 15s timeout | (a) State collector: CloudWatch ΓåÆ 23-dim state ΓåÆ S3; (b) Scaling trigger: ASG scale-out/in | First 1M requests/mo free ΓåÆ **effectively free** for our use |
| 4 | CloudWatch | Built-in + 8 custom metrics | CPUUtilization (built-in, free); 8 custom metrics (QueueDepth + ResponseTimeEMA ├ù 4 instances) | Built-in metrics free; 8 custom metrics within the 10-metric free tier ΓåÆ **free** |
| 5 | S3 | 1 bucket, ~2ΓÇô4 GB total | Model checkpoints, state buffer (`state/current.json`), experiment results | < 5 GB ΓåÆ **free tier** |
| 6 | DynamoDB | 1 table, on-demand | Routing decision log (action, state, experiment_id, timestamp) | < 25 GB, < 200M requests ΓåÆ **free tier** |
...
| 9 | SNS | 1 topic | Billing alarm email notification | First 1M notifications free ΓåÆ **free** |
| 10 | SageMaker *(optional fallback)* | ml.t2.medium notebook | PPO/DQN training **only if** local laptop or Colab fails | $0.046/hr; stop immediately after use; ~$0.20ΓÇô$0.50 if used |
| 11 | API Gateway *(optional entry point)* | REST API | Optional front-door for JMeter; not in the critical experiment data path | First 1M calls/mo free; removed from primary flow ΓÇö JMeter targets ALB DNS directly |

**Removed from architecture:**
- **CloudFront** ΓÇö removed entirely; provides no research value for load-balancing experiments.
- **API Gateway as primary entry point** ΓÇö removed from critical data path; JMeter directs traffic to ALB DNS.

### 12.2 Exact Role of Each Service

**EC2 (Backend servers, t2.micro ├ù 4):**
- Run a lightweight Flask application simulating an e-commerce product-catalogue endpoint
- CPU-intensive endpoint: `GET /product/<id>` performs a 10ΓÇô50ms computation to simulate a DB query
...
- Stopped (not terminated ΓÇö to preserve EBS) between experiment sessions

**EC2 (Inference server, t2.micro ├ù 1):**
...
- Stopped between experiment sessions; started ΓëÑ 60s before experiments begin (model load time)

**ALB:**
- Single HTTP listener on port 80 ΓåÆ forwards to `FlashBalanceAI-TG` (weighted target group)
...
**Lambda ΓÇö State Collector (30s trigger via EventBridge):**
```python
# Pseudo-code
def collect_state():
    # Reads 4 built-in CW metrics (CPUUtilization) + 8 custom metrics
    # (QueueDepth ├ù 4, ResponseTimeEMA ├ù 4) from CloudWatch
...
**Lambda ΓÇö Inference Coordinator (30s trigger via EventBridge):**
```python
# Pseudo-code ΓÇö lightweight; does NOT load the PPO model
def coordinate():
...
**Lambda ΓÇö Scaling Trigger (CloudWatch alarm trigger):**
```python
# Pseudo-code
...
**CloudWatch ΓÇö Metrics in use (8 custom + built-in):**
...
- Custom (8 metrics, within 10-metric free tier): `FlashBalanceAI/Instances QueueDepth` ├ù 4 + `FlashBalanceAI/Instances ResponseTimeEMA` ├ù 4
- Alarms (5 free): `HighCPU-Alarm` (CPU > 70%, 2├ù30s), `LowCPU-Alarm` (CPU < 30%, 5├ù30s), `BillingAlarm-$5`, `BillingAlarm-$20`, `Integration-Watchdog`
...
**S3 ΓÇö Bucket layout:**
```
flashbalanceai-{account_id}/
Γö£ΓöÇΓöÇ models/
Γöé   Γö£ΓöÇΓöÇ ppo_flash_v1.zip        (~50 MB)
Γöé   ΓööΓöÇΓöÇ dqn_flash_v1.zip        (~50 MB)
Γö£ΓöÇΓöÇ state/
Γöé   ΓööΓöÇΓöÇ current.json            (< 1 KB, overwritten every 30s)
Γö£ΓöÇΓöÇ results/                    (JMeter .jtl files + CloudWatch exports)
ΓööΓöÇΓöÇ configs/                    (runtime config copies)
```
Expected total: ~2ΓÇô3 GB. Well within 5 GB free tier.
...
- If used: ~$0.20ΓÇô$0.50 total per training run
- **Primary path:** local laptop CPU (~3ΓÇô4 hrs) or Google Colab free tier

**API Gateway *(optional entry point ΓÇö removed from critical path)*:**
...
- On-demand billing: free tier covers 25 GB storage + 200M requests/month ΓåÆ **free**
...
```
JMeter (local) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓû║ ALB DNS (HTTP:80)
                                                  Γöé
                                    ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓû╝ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ
                                    Γöé  EC2 Backend Pool (Flask)  Γöé
                                    Γöé  server-0 ΓÇª server-3       Γöé
                                    Γöé  t2.micro ├ù 4              Γöé
                                    ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö¼ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ
                                                  Γöé (every 30s)
                                    ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓû╝ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ
                                    Γöé  CloudWatch                Γöé
                                    Γöé  CPUUtil (built-in)        Γöé
                                    Γöé  QueueDepth ├ù 4 (custom)   Γöé
                                    Γöé  ResponseTimeEMA ├ù 4       Γöé
                                    ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö¼ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ
                                                  Γöé
                            ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓû╝ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ
                            Γöé  Lambda: State Collector (every 30s)       Γöé
                            Γöé  ΓåÆ builds 23-dim state ΓåÆ S3 current.json   Γöé
                            ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö¼ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ
                                                  Γöé
                            ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓû╝ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ
                            Γöé  Lambda: Inference Coordinator (every 30s) Γöé
                            Γöé  ΓåÆ reads S3 state ΓåÆ POST /infer to EC2     Γöé
                            Γöé  ΓåÆ EC2 inference server returns action      Γöé
                            Γöé  ΓåÆ boto3 elbv2.modify_target_group_attrs   Γöé
                            Γöé  ΓåÆ DynamoDB log                            Γöé
                            ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö¼ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ
                                                  Γöé (CPU alarm triggers)
                            ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓû╝ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ
                            Γöé  CloudWatch Alarm ΓåÆ Lambda: Scaling TriggerΓöé
                            Γöé  ΓåÆ ASG SetDesiredCapacity                  Γöé
...
- CloudWatch Logs: Lambda invocation logs ΓÇö state collection, inference coordinator, scaling trigger (7-day retention)
...
- Local Python/Dash monitoring dashboard (`src/metrics/visualiser.py`) ΓÇö runs on experimenter's laptop, reads `state/current.json` from S3 every 5s, no AWS dashboard cost
...
- No Lambda model loading (Lambda size limit constraint ΓÇö SB3 + PyTorch Γëê 300 MB unzipped)
...
| Weighted Round Robin (WRR) | Weight Γê¥ instance capacity | Slightly smarter RR; Agrima's comparison |
| Least Connections (LC) | Route to instance with fewest active connections | Common production heuristic |
| Threshold-Based Autoscaling | AWS CloudWatch alarm: CPU > 70% ΓåÆ scale out, CPU < 30% ΓåÆ scale in | Current production standard; Devkanti's primary comparison baseline |
...
|--------|-----------|--------------|-------------|
| Average Latency | Mean response time across all requests in experiment window (ms) | < 200ms at 10├ù burst | CloudWatch `TargetResponseTime` + JMeter |
| P95 Latency | 95th percentile response time (ms) | < 500ms at 10├ù burst | JMeter aggregate report |
| P99 Latency | 99th percentile response time (ms) | < 1000ms at 10├ù burst | JMeter aggregate report |
| Throughput | Successfully served requests per second (req/s) | Maximised | JMeter throughput counter |
| Request Failure Rate | HTTP 5xx errors / total requests (%) | < 1% at 10├ù burst | JMeter error summary |
| SLA Violation Rate | % of requests with latency > SLA threshold (500ms) | < 5% at 10├ù burst | Computed from JMeter results |
| CPU Utilisation (avg) | Mean CPU across all active instances during peak (%) | 70ΓÇô85% | CloudWatch `CPUUtilization` |
...
| Cost per 1M Requests | Total AWS cost during experiment / requests served ├ù 10Γü╢ | 20ΓÇô25% lower than static baseline | AWS Cost Explorer |
| Total AWS Cost | Absolute cost during experiment window | Minimised | AWS Cost Explorer |

### 14.2 Statistical Analysis Requirements

- **Repetitions:** Each experiment repeated N_rep ΓëÑ 5 times with different seeds
- **Reported values:** Mean ┬▒ standard deviation for all metrics
- **Confidence intervals:** 95% CI using t-distribution (small sample)
- **Significance testing:** Paired t-test (PPO vs each baseline) on P95 latency; reject HΓéÇ at ╬▒ = 0.05
...
| Traffic type | Generic microservices | General serverless | General IoT tasks | General edge computing | Batch jobs | Flash-sale burst (10├ùΓÇô100├ù) |
...
| Flash-sale traffic | Γ£ù | Γ£ù | Γ£ù | Γ£ù | Γ£ù | Γ£ô |
| P95/P99 latency metric | Γ£ù | Γ£ù | Γ£ù | Γ£ù | Γ£ù | Γ£ô |
| Cost optimisation | Γ£ù | Partial (over-provisioning penalty) | Γ£ù | Γ£ù | Γ£ù | Γ£ô (multi-objective reward) |
| DQN vs PPO comparison | Γ£ù | Γ£ù | Γ£ù | Γ£ô (SDN sim only) | Γ£ù | Γ£ô (on AWS, flash-sale) |
| AWS-native deployment | Γ£ù | Γ£ù | Γ£ù | Γ£ù | Γ£ù | Γ£ô |
| Proactive pre-scaling | Γ£ù | Γ£ù | Γ£ù | Γ£ù | Γ£ù | Γ£ô |
| Reproducibility | Partial | Γ£ô (code shared) | Γ£ô (GitHub) | Partial | Partial | Γ£ô (open-source, fixed seeds) |
...
**Verified Gap G1:** No existing DRL-based load balancing work specifically targets flash-sale burst traffic characterised by 10├ùΓÇô100├ù request rate escalation within seconds. All surveyed papers use general workloads (batch jobs, generic microservices, serverless, SDN edge).
...
**Verified Gap G3:** Only one surveyed paper (Zhou et al., 2024, Elsevier) compares DQN and PPO on the same platform, and this comparison is in an SDN edge simulation ΓÇö not cloud, not flash-sale traffic.
...
**Verified Gap G5:** P95/P99 latency metrics are absent from all surveyed papers except Yamsani et al. (2026) ΓÇö and that work does not test under flash-sale conditions.

**Unverified / Requires Validation:**
- "First DRL load balancer on AWS" ΓÇö **do not claim without a comprehensive literature search beyond these 10 papers**.
- Exact percentage improvements (30% P95 reduction) ΓÇö **these are EXPECTED results, not validated**.
...
|-------|--------|-------------|
| First DRL load balancer specifically for flash-sale burst traffic | Devkanti, Agrima | Partially ΓÇö within the 10 surveyed papers. Cannot claim globally without broader search. |
| Novel 4-component burst-aware reward function | Devkanti | Defensible ΓÇö specific combination of latency + cost + throughput + SLA penalty for flash-sale context is novel |
| DQN vs PPO comparison on AWS flash-sale setup | Agrima | Defensible ΓÇö not done in surveyed literature |
| DDPG + H-MAS integration | Mohar | NOT RETAINED ΓÇö too speculative |
| AWS-native DRL deployment (ELB, ASG, Lambda, CloudWatch) | Devkanti, Agrima | Defensible within surveyed scope |
| Proactive pre-scaling before burst onset | Devkanti | Defensible as combination ΓÇö not seen in surveyed papers |
| 30% P95 reduction, 85% utilisation, 20ΓÇô25% cost saving | Devkanti | EXPECTED RESULTS ONLY ΓÇö not yet experimentally validated |

### 17.2 Defensible Novelty After Verification

**N1. Flash-Sale Burst-Aware DRL Load Balancing on AWS**
Within the surveyed literature (10 papers, top-tier venues, 2023ΓÇô2026), no work combines: (a) DRL-based per-request routing, (b) AWS-native deployment (ALB + ASG + Lambda + CloudWatch), and (c) evaluation under flash-sale burst traffic profiles. This combination is the primary novelty claim.

**N2. Four-Component Multi-Objective Burst-Aware Reward Function**
The specific combination of response time penalty, utilisation reward, throughput fraction, and SLA violation penalty ΓÇö tuned and validated for flash-sale burst conditions ΓÇö constitutes a novel reward engineering contribution.
...
The `burst_indicator` state feature enables the agent to trigger pre-scaling before CPU saturation ΓÇö a behaviour not achievable with standard threshold-based CloudWatch alarms.

### 17.3 Claims to NOT Make

- "No existing work addresses DRL for cloud load balancing" ΓÇö false; the field has extensive literature
- "First to use PPO for cloud autoscaling" ΓÇö false; Femminella & Reali (2024) precede this work
- "First to use DDPG for continuous resource allocation" ΓÇö false; well-established
- Any specific numerical results (30%, 85%, 20ΓÇô25%) until experimentally validated
...
**ΓÜá∩╕Å ALL RESULTS BELOW ARE EXPECTED (UNVALIDATED HYPOTHESES). NONE ARE EXPERIMENTALLY CONFIRMED.**

| Hypothesis | Expected Direction | Basis |
|------------|-------------------|-------|
| H1: PPO outperforms Round Robin on P95 latency at 10├ù burst | PPO ΓëÑ 30% lower P95 | MDP formulation should capture burst dynamics; RR ignores server state |
| H2: PPO outperforms Threshold-based autoscaling on P95 latency | PPO ΓëÑ 30% lower P95 | Proactive pre-scaling avoids cold-start delay |
| H3: PPO achieves higher CPU utilisation than static baselines | 70ΓÇô85% for PPO vs ~40ΓÇô50% for static | Multi-objective reward penalises under-utilisation |
| H4: PPO reduces AWS cost vs static over-provisioned baseline | 20ΓÇô25% cost reduction | Fewer idle instances needed |
| H5: PPO outperforms DQN on P95 latency at 50├ù burst | PPO marginally better, within 10% | PPO's policy gradient stability should benefit under high reward variance |
| H6: All DRL methods degrade gracefully at 100├ù burst | All methods exceed SLA; PPO degrades least | Infrastructure ceiling limits all methods |
| H7: Ablation: removing SLA penalty from reward increases P99 latency | +20ΓÇô40% P99 increase | SLA term is responsible for tail latency control |
| H8: Proactive pre-scaling reduces scaling reaction time | 50ΓÇô70% reduction in time-to-scale | Burst indicator triggers early ASG call |
...
5. **N=4 scale limit:** Student account limits evaluation to 4ΓÇô8 instances. Results for hundreds of instances cannot be extrapolated without further validation.
...
**H1 (Formal):** HΓéÇ: ╬╝(P95_PPO) ΓëÑ ╬╝(P95_Threshold). HΓéü: ╬╝(P95_PPO) < ╬╝(P95_Threshold). Test: paired t-test, ╬▒ = 0.05, N_rep ΓëÑ 5.
...
3. **Related Work**: 8ΓÇô10 papers in 3 groups: (a) DRL for cloud scheduling, (b) DRL for load balancing, (c) AWS/cloud autoscaling; explicit gap identification
...
7. **Results**: Table: all baselines ├ù all metrics (E1ΓÇôE9); graphs: latency CDF, reward convergence, cost comparison, scaling timeline; ablation table (E10)
8. **Discussion**: Answer RQ1ΓÇôRQ5; explain unexpected results; H-MAS insight from Mohar's work as future direction
9. **Limitations and Threats to Validity**
10. **Conclusion**: Summary of contributions; future work (DDPG, multi-region, real production data)
11. **References**: 15ΓÇô20 papers; include all 10 surveyed papers
...
|-----|------|---------|--------------|
| E1 | Baseline Traffic (1├ù load) | Confirm all systems work; calibrate baselines | 5 |
| E2 | 10├ù Traffic Spike | Primary comparison: all baselines | 5 |
| E3 | 50├ù Traffic Spike | Stress test; expected degradation in simple baselines | 5 |
| E4 | 100├ù Traffic Spike | Upper bound; expected infrastructure ceiling | 3 |
| E5 | Repeated Flash Sales | Multiple spikes within one episode; recovery behaviour | 5 |
| E6 | Noisy/Transient Spikes | Short-lived 2├ù spikes within baseline; test over-reaction | 5 |
| E7 | Cold-Start / Scaling Response | Measure time-to-scale for PPO vs threshold autoscaling | 5 |
| E8 | Cost/Performance Trade-off | Vary wΓéé (utilisation weight) and measure cost vs P95 | 3 per config |
...
**Minimum total experiment budget:** E1+E2+E3+E4+E5+E6+E7+(E8├ù3)+E9+E10 Γëê **60ΓÇô80 runs**
...
### DRL Algorithm
**PPO (Proximal Policy Optimization)** with 2-layer MLP actor-critic [64, 64], trained using Stable-Baselines3. Discrete action space (N=4 backend instances). Clip ratio 0.2, GAE ╬╗=0.95, 2M training steps.
...
EC2 (t2.micro ├ù 4), ALB (1), Lambda (3 functions), CloudWatch (custom metrics), S3 (1 bucket), SageMaker (notebook ml.t2.medium, stopped after training), API Gateway (REST), ASG (1 group), IAM (2 roles), DynamoDB (1 table).

### Dataset / Traffic
Synthetic FlashSale-Traffic-Synthetic v1.0 generated by `traffic_generator.py`. Calibrated to 10├ù and 50├ù burst multipliers. No real production data required. Alibaba Cluster Trace used only for supplementary pre-training analysis.
...
### Experiments
E1ΓÇôE10 as specified in Section 24, minimum 5 repetitions each, 95% CI, paired t-test for RQ1/RQ2.
...
**Estimated total AWS cost for full experimental campaign: $15ΓÇô35 USD** (within student credit budget)
